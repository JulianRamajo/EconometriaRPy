...
::: justify
# Aplicación 3.12 (Variable dependiente discreta o limitada): Préstamos hipotecarios. Compra de automóviles. Salario de las mujeres casadas {.unnumbered}

## Variable dependiente discreta (modelos Logit y Probit): Elección entre tipo de interés fijo o variable {.unnumbered}
...
$$Y_{i} = \beta_1 + \beta_2 FI_{i} + \beta_3 MARG_{i} + \beta_4 YLD_{i} + \beta_5 PTS_{i} + \beta_6 MAT_{i} +$$
$$+ \beta_7 BA_{i} + \beta_8 BS_{i} + \beta_9 FTB_{i} + \beta_{10} CB_{i} + \beta_{11} MC_{i} + $$
$$+ \beta_{12} SE_{i} + \beta_{13} MOB_{i} + \beta_{14} NW_{i} + \beta_{15} LA_{i} + \beta_{16} STL_{i} + e_{i}$$
:::
...
::: justify
## Variable dependiente limitada (valores censurados -> modelo Tobit): Compra de automóviles {.unnumbered}
...
::: justify
## Variable dependiente limitada (truncamiento selectivo -> modelo Heckit de selección muestral): Salarios de las mujeres casadas en el Reino Unido {.unnumbered}
