...
::: justify
# Aplicación 3.12 (Variable dependiente discreta o limitada): Modelos Logit/Probit, Tobit y Heckit {.unnumbered}

## Variable dependiente discreta (modelos Logit y Probit): Préstamos hipotecarios {.unnumbered}
...
modelo_logit_2 <- glm(Y ~ FI + MARG + YLD, 
                      data=PREST_HIP, family=binomial(link="logit"))
summary(modelo_logit_2)
# Matriz de convarianzas robusta
...
margEff(modelo_tobit_1, 
        xValues = c(1,mean(AUTOS$EDAD),mean(AUTOS$HIJOSM18),mean(AUTOS$Y)))
...
::: justify
## Variable dependiente limitada (truncamiento selectivo -\> modelo Heckit de selección muestral): Salarios de las mujeres casadas {.unnumbered}
