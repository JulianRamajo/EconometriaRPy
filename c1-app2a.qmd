...
pSAN |>
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Precio de las acciones del Santander desde el año 2000 (EUR)"
  )
...
# ANEXO: Librerías especializadas en análisis de series temporales y 
# datos financieros
#
# Lectura de librerías
library(tidyverse)
...
head(SP500,10)
FAANG_prices <- tq_get(c("META", "AAPL", "AMZN", "NFLX", "GOOG"), 
                       get = "stock.prices", 
                       from = "2015-01-01", 
                       to = "2023-11-30")
FAANG_prices %>%
  ...
  plot_time_series(date, adjusted, .interactive = FALSE) # series y tendencias
# Agrupación de series temporales de precios
FAANG_prices_ts <- FAANG_prices %>%
  group_by(symbol) %>%
  select(symbol, date, adjusted) %>%
  pivot_wider(names_from = symbol, values_from = adjusted)
ts_plot(FAANG_prices_ts, 
        title = "Precio de las acciones del grupo FAANG", 
        Ytitle = "Index")
# Si se quiere una periodicidad anual, mensual o semanal
# se pueden usar el comando tq_transmute.
# Opciones disponibles:
# tq_transmute_fun_options() %>% str()
# tq_mutate_fun_options()
FAANG_daily <- FAANG_prices %>% 
  group_by(symbol) %>% 
  select(symbol, date, adjusted) 
FAANG_monthly <- FAANG_daily %>%   
...
FAANG_monthly %>% 
  ...
  labs(title = "Precio de las acciones del grupo FAANG",
       x = "Meses", y = "Precios corregidos", color = "") +
...
FAANG_yearly_returns %>% ggplot(aes(x = date, y = yearly.returns, 
                                    fill = symbol)) +
  geom_col() +
  geom_hline(yintercept = 0, color = palette_light()[[1]]) +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Rendimientos anuales del grupo FAANG", 
       subtitle = "X", y = "Rendimiento", x = "") + 
  facet_wrap(~ symbol, ncol = 2, scales = "free_y") +
  theme_tq() + 
  scale_fill_tq()
...
FAANG_daily %>% 
  ...
  geom_line(linewidth = 1) +
  labs(title = "Precio de las acciones del grupo FAANG",
       x = "Días", y = "Precios corregidos", color = "") +
  facet_wrap(~ symbol, ncol = 2, scales = "free_y") +
  scale_y_continuous(labels = scales::dollar) +
  theme_tq() + 
  scale_color_tq()
FAANG_daily_returns <- FAANG_prices %>% 
  group_by(symbol) %>% 
  tq_transmute(select = adjusted, mutate_fun = periodReturn, 
               period = "daily", type  = "log")
# Rentabilidad
FAANG_daily_returns %>% ggplot(aes(x = date, y = daily.returns, 
                                   fill = symbol)) + geom_col() +
  geom_hline(yintercept = 0, color = palette_light()[[1]]) +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Rendimiento diario de las acciones del grupo FAANG", 
...
FAANG_daily_returns %>% ggplot(aes(x = daily.returns, fill = symbol)) +
  ...
  labs(title = "Distribución de los rendimientos diarios del grupo FAANG",  
       subtitle = "(Log-returns)", 
       x = "", y = "Densidad") +
  theme_tq() +
  scale_fill_tq() + 
...
pSAN = yf.download("SAN.MC", start="2000-01-01", end="2023-11-30")
print(pSAN)
