...
library(dplyr)
library(readr)
# Number of questions asked by month for both Julia, Python and R:
# https://data.stackexchange.com/stackoverflow/query/1599213/r-python-julia-questions
...
# If we standardise Python to 1, we see that R and Julia are growing relative to python over time
rel <- so %>% 
  group_by(month) %>% 
...
ggplot(rel, aes(month, rel)) + 
  geom_line() + 
  ylab("Julia questions as proportion of python questions")
