...
Desde el punto de vista metodológico, el objetivo de esta aplicación es la introducción del uso de *variables ficticias* en el modelo de regresión lineal, mostrando su potencial para modelizar variables explicativas de tipo cualitativo.

Desde el punto de vista económico, estamos interesados en encontrar pruebas a favor o en contra de la discriminación de las mujeres en el mercado laboral americano. Compararemos los salarios de hombres y mujeres que comparten similitudes en factores salariales relevantes para la determinación del salarios tales como la edad o el nivel de educación, por ejemplo, aunque extenderemos dichos elementos con diferentes variables de control para tener una aproximación más realista del concepto *ceteris paribus* (condicionar sobre \`todas' las variables relevantes para poder aproximarse a la verdadera relación causal).

Como especificación básica se usará la siguiente [ecuación de salarios de Mincer](https://es.wikipedia.org/wiki/Funci%C3%B3n_de_ingresos_de_Mincer):
...
library(corrplot)
# Lectura de datos
cps <- read_csv("data/MORG14_USA.csv")
#
...
# Lectura de datos
cps = pd.read_csv('data/MORG14_USA.csv')
# Filtrado
