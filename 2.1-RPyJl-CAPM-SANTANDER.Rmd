...
subtitle: 'Aplicación 2.1 (estimación y contrastes de hipótesis): Modelo CAPM para las acciones del Banco Santander'
output:
  html_document:
    theme: journal
    highlight: haddock
...
$$erSAN_t = \beta_1 + \beta_2 {erIBEX_t} + e_t$$

donde _erSAN_ representa el 'exceso de rendimiento' de las acciones del Banco Santander (rentabilidad de las acciones del banco menos rentabilidad de un activo libre de riesgo, las letras del tesoro) y _erIBEX_ es el exceso de rendimiento del mercado (representando por el índice IBEX35 de la bolsa española).
...
CAPM_df = DataFrame(load("CAPM_ESP.csv", delim = ';'))
# Estructura de series temporales
using TimeSeries
CAPM_ta = TimeArray(CAPM_df, timestamp = :date)
# Gráficas
using Plots
plot(CAPM_ta[:P_IBEX35])
plot(CAPM_ta[:P_SANTANDER])
# log-returns
percentchange(CAPM_ta[:P_SANTANDER], :log)
plot(percentchange(CAPM_ta[:P_SANTANDER], :log))
# Fichero con tasas mensuales
SANTANDER_ta = merge(percentchange(CAPM_ta[:P_SANTANDER],:log).*100, percentchange(CAPM_ta[:P_IBEX35],:log).*100, CAPM_ta[:R_LT1Y]./12)
#
SANTANDER_df = DataFrame(SANTANDER_ta)
#
using Query
SANTANDER_df = SANTANDER_df |> @rename(:P_SANTANDER => :R_SAN, :P_IBEX35 => :R_IBEX)
#
SANTANDER_df = SANTANDER_df |> @mutate(ER_SAN = (_.R_SAN - _.R_LT1Y) , ER_IBEX = (_.R_IBEX - _.R_LT1Y))
#
...
using GLM
CAPM_SANTANDER = lm(@formula(ER_SAN ~ ER_IBEX), SANTANDER_df)
```
