---
title: 'R, Python y Julia usando RStudio'
author:
  name: Julián Ramajo, ramajo@unex.es
  affiliation: GRADO EN ESTADÍSTICA | ECONOMETRIA (502243)
subtitle: 'Aplicación 0: Instalación y configuración del software'
output:
  html_document:
    theme: journal
    highlight: haddock
...
# Configuración de Python en RStudio
...
O desde R se pueden ejecutar algunos comandos Python importando la librería correspondiente:
...
# Configuración de Julia en RStudio
...
Estos comandos hay que ejecutarlos cada vez que se use Julia en RStudio:
...
O desde R se pueden ejecutar algunos comandos Julia para hacer los mismos cálculos:
...
También se pueden ejecutar comandos y realizar gráficas en `Julia` y mostrarlos en `R`:

```{julia}
a = sqrt(2)
# Si no está instalada la librería concreta se puede instalar previamente
...
## Creación de un entorno Julia particular (Opcional, para aislar entornos de trabajo)
