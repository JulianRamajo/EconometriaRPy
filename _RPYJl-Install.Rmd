...
4. Instalar la librería `Rcpp` y el software **Rtools** ( <https://cran.r-project.org/bin/windows/Rtools/> )
...
5. Una vez instalados R y RStudio, el siguiente paso consiste en **instalar las librerías** específicas para realizar análisis estadísticos y econométricos.
...
O se puede ejecutar un script de `Python` y mostrar los resultados en `R`:
...
Crear un _entorno conda_ propio es tan sencillo como generar un nuevo nombre del mismo, tal y como se muestra a continuación:
...
El siguiente paso consiste en instalar las librerías de Python pertinentes en el entorno de R-Miniconda recién creado. La razón por la que queremos usar `reticulate` es para poder _acceder a todas las librerías de Python que instalemos desde R_.
...
# Instalación y onfiguración de Julia en RStudio

Para poder usar Julia en RStudio, en primer lugar hay que instalar la librería **JuliaCall**,  y luego utilizar instalar Julia e inicializarlo:

```{r}
install.packages("JuliaCall")
```

Los comandos siguiente hay que ejecutarlos cada vez que se use Julia en RStudio:
...
Para comprobar que el entorno Julia ha sido creado adecuadamente se puede entrar en la consola de Julia
y hacer algún pequeño cálculo para ver si la instalación ha sido correcta:
...
También desde R se pueden ejecutar algunos comandos Julia para hacer los mismos cálculos:
...
O se pueden ejecutar comandos y realizar gráficas en `Julia` y mostrarlos en `R`:
...
También podría ejecutarse la gráfica en el script de Julia en R, sin error, pero no se verá la gráfica ni en la pantalla ni en el R Markdown.
...
Un ejemplo de econometría usando Julia en RStudio:
...
# Instalación separada del software y configuración de Jupyter 

1. Instalar R y RStudio
   Instalar Python y Julia
    
2. Instalar Jupyter: Ir a Terminal de MacOS/Windows —>
	...
	Opcional: python3 -m pip install notebook (py -m pip install notebook)
	
3. Instalación de kernels de Julia y R:

3a. Ir al Terminal de Julia ->
	Pulsar ]
	add IJulia
	Pulsar Control+c

3b. Entrar en RStudio o en R para instalar IRkernel ->
	Install.packages("IRkernel")
	En el Terminal de MacOS entrar en R (en Windows entrar en R directamente)
	IRkernel: installspec()

4. Para comprobar que todo ha ido bien, ir a la Terminal de MacOS o Windows ->
	python3 -m jupyter lab (py -m jupyter lab)
	
5. Instalar las librerías correspondientes en R, Python y Julia ->
  R -> install.packages("*")
  Python -> En el terminal de macOS/Windows -> python3 -m pip install *
  Julia -> En el terminal de julia -> pulsar ] -> add * -> pulsar Control+c
