...
#
# O también: 
fit.KEYNES_model <-  CY_USA %>% model(tslm = TSLM(C ~ Y))
report(fit.KEYNES_model)
#
# Ajuste del modelo
#
augment(fit.KEYNES_model) %>%
  ggplot(aes(x = Year)) +
...
#
glance(fit.KEYNES_model) %>% select(r_squared, adj_r_squared, CV, AIC, AICc, BIC)
# 
# Evaluación del modelo estimado
#
# Autocorrelación y normalidad de los errores (a trevés de los residuos estimados)
#
fit.KEYNES_model %>% gg_tsresiduals()
#
augment(fit.KEYNES_model) %>% features(.resid, ljung_box, lag = 10, dof = 2)
#
# Residuos frente a predictores (no lienalidad o mala especificación funcional)
#
tmp <- left_join(CY_USA, residuals(fit.KEYNES_model), by = "Year")
...
# No estacionariedad (raíces unitarias) -> Modelo con tendencia determinística
#
# CY_USA %>%
