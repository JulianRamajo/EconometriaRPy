...
::: justify
# Aplicación 3.13: Modelos econométricos para datos de panel {.unnumbered}

## Productividad de la industria química en China {.unnumbered}
...
# Modelos estáticos para datos de panel
# Modelo plano
modelo_plano = PooledOLS.from_formula('np.log(Y) ~ 1 + np.log(K) + np.log(L) + np.log(M)', 
QUIM_CHINA_pdata)
res = modelo_plano.fit()
print(res)
# Modelo de efectos fijos
modelo_ef = PanelOLS.from_formula('np.log(Y) ~ 1 + np.log(K) + np.log(L) + np.log(M) + EntityEffects', 
QUIM_CHINA_pdata)
res = modelo_ef.fit()
print(res)
# Modelo de efectos aleatorios
modelo_ea = RandomEffects.from_formula('np.log(Y) ~ 1 + np.log(K) + np.log(L) + np.log(M)', 
QUIM_CHINA_pdata)
res = modelo_ea.fit()
print(res)
```
