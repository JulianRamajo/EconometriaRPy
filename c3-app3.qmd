...
::: justify
# Aplicación 3.3 (Estabilidad de los parámetros estructurales): Exportaciones españolas. Diferenciación salarial por sexo. Gasto en construcción {.unnumbered}
...
### Diferencia salarial por sexo {.unnumbered}
...
### Exportaciones españolas {.unnumbered}
...
### Inversión en construcción en Estados Unidos {.unnumbered}
...
TCS <- read_csv("data/TTLCON_USA.csv") %>% setNames(c("date","y"))
head(TCS)
tail(TCS)
class(TCS)
str(TCS)
...
import statsmodels.stats as smstats
# Lectura de datos
EXP_ESP = pd.read_csv("data/EXP_ESP_Y.csv", sep=";", parse_dates=['date'], index_col='date')
