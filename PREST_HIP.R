...
attach(PREST_HIP)
# Modelo logit
ti.logit <- glm(Y ~ FI + MARG + YLD + PTS + MAT + BA + BS + FTB + CB + MC + SE + MOB + NW + LA + STL, family=binomial(link="logit"))
summary(ti.logit)
# Modelo probit
ti.probit <- glm(Y ~ FI + MARG + YLD + PTS + MAT + BA + BS + FTB + CB + MC + SE + MOB + NW + LA + STL, family=binomial(link="probit"))
summary(ti.probit)
...
#
ti.logit.1 <- glm(Y ~ FI + MARG + YLD + PTS + MAT, family=binomial(link="logit"))
summary(ti.logit.1)
linearHypothesis ( ti.logit.1 , c("PTS = 0", "MAT = 0"))
ti.logit.2 <- glm(Y ~ FI + MARG + YLD , family=binomial(link="logit"))
S(ti.logit.2)
...
lines(FInew, predict(ti.logit.2, newdata=data.frame(FI=FInew, MARG=rep(mean(MARG), 78), YLD=rep(mean(YLD), 78)), type="response"), lwd=1.5)
...
lines(MARGnew, predict(ti.logit.2, newdata=data.frame(FI=rep(mean(FI), 78), MARG=MARGnew, YLD=rep(mean(YLD), 78)), type="response"), lwd=1.5)
grid(col="gray", lty="solid")
#
...
lines(YLDnew, predict(ti.logit.2, newdata=data.frame(FI=rep(mean(FI), 78), MARG=rep(mean(MARG), 78), YLD=YLDnew), type="response"), lwd=1.5)
grid(col="gray", lty="solid")
