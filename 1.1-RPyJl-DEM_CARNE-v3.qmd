...
Inicialización de Python (`reticulate`) y Julia (`JuliaCall`) para poder ejecutar simulténeamente la aplicación en los tres lenguajes.
...
```{julia}
lin_model_2 = fit(EconometricModel, @formula(Q ~ P + Y), DEM_CARNE)
```
:::
