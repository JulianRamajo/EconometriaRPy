...
::: justify
# Aplicación 3.5: Regresiones heteroscedásticas {.unnumbered}

## Función de demanda con varianza no constante {.unnumbered}
...
Una vez chequeada la hipótesis de varianza constante (homoscedasticidad), se plantea la estimación por mínimos cuadrados ponderados (MCP) de la función de demanda bajo la hipótesis de un modelo heteroscedasticidad asociada al logaritmo de la renta del tipo siguiente:

$${σ_i}^2 = e^{α_0 + α_1 log⁡(Y_i)}$$

Esta especificación para la varianza residual equivale a suponer que
...
# Corrección avanzada: mínimos cuadrados ponderados (MCP <-> FGLS)
...
DEM_HET.describe()
# Estimación MCO
modelo1 = smf.ols(formula = 
"np.log(QA) ~ np.log(PA) + np.log(PB) + np.log(PC) + np.log(Y)", 
data = DEM_HET)
...
w=list(1/sigma2)
# MCO con ponderaciones
modelo2 = smf.wls(formula = 
"np.log(QA) ~ np.log(PA) + np.log(PB) + np.log(PC) + np.log(Y)", 
weights = w , data = DEM_HET)
