...
cps <- cps %>% mutate(ed_MA=as.numeric(grade92==44), ed_PSD = as.numeric(grade92==45), ed_DD = as.numeric(grade92==46))
reg4 <- lm(log(w) ~ female + age + age2 + age3 + age4 + ed_PSD + ed_DD, data=cps)
...
reg5 <- lm(log(w) ~ female + age + age2 + age3 + age4 + ed_PSD + ed_DD + afram + hisp + asian + othernonw + nonUSborn + married + divorced + wirowed + child1 + child2 + child3 +child4pl + uhours + fedgov + stagov + locgov + nonprof + union + as.factor(stfips) + as.factor(ind2dig) + as.factor(occ2dig), data=cps)
# COMPARACIÓN FINAL
