...
En esta aplicación se analizarán los datos de la última publicación del [*Global Data Lab*](https://globaldatalab.org/) sobre las variaciones habidas en el desarrollo humano, la salud, la educación y la renta dentro de más de 160 países ('*within*') y entre ellos ('*between*') desde 1990 hasta 2021.

Puede encontrarse toda la información detallada sobre el índice de desarrollo humano al nivel subnacional (SHDI en sus siglas en inglés) en la siguiente dirección: <https://www.nature.com/articles/sdata201938>; y también se puede acceder a los datos originales desde esta otra página: <https://globaldatalab.org/shdi/download_files/>.
...
df1
colnames(df1)
# Selección de los años inicial y final
df1$year <- as.character(df1$year)
df1990 <- filter(df1, year == '1990')
print(df1990)
...
df2021 %>%
  ...
  filter(country == 'Spain') %>% 
  summary()
# Box plots: diferencias regionales
...
# Series temporales: evolución regional dentro de un país
plot_ly(df1[df1$country == "Spain",], x = ~year, y = ~shdi, color = ~region, type = "scatter", mode = "lines") %>%
    layout(xaxis = list(title = "Año"),
           yaxis = list(title = "SHDI", type = "log"))
# Scatter plots (gráficos de dispersión X-Y): correlaciones
plot_ly(df1, 
        y = ~shdi, 
        x = ~lgnic,
        frame = ~year,
...
# Mapas de distribución espacial del SHDI
df1990_2 <- df1990 %>% 
  rename(GDLcode = GDLCODE)
df2021_2 <- df2021 %>% 
  rename(GDLcode = GDLCODE)
# Año 1990
gdf1990 <- inner_join(map, df1990_2, by = "GDLcode")
colnames(gdf1990)
print(gdf1990)
ggplot(gdf1990) +
  geom_sf(aes(fill=shdi)) +
  theme_bw() +
  labs(title = "Distribución espacial del SHDI en 1990") +
...
ggplot(gdf2021Spain) + geom_sf(aes(fill=shdi)) +
  ...
  scale_fill_viridis(option="OrRd")
# Sólo las regiones peninsulares
...
df1990.describe().round(3)
df1990.query("country == 'Spain'").describe().round(3)
df2021.describe().round(3)
#df2021['shdi'].describe().round(3)
...
# Análisis exploratorio espacial (ESDA)
# Mapas de distribución espacial del SHDI
df1990_2 = df1990.copy()   
df1990_2 = df1990_2.rename(columns = {"GDLCODE": "GDLcode"})
df2021_2 = df2021.copy()   
df2021_2 = df2021_2.rename(columns = {"GDLCODE": "GDLcode"})
gdf1990 = map.merge(df1990_2, on = 'GDLcode', how = 'left')
gdf2021 = map.merge(df2021_2, on = 'GDLcode', how = 'left')
# Año 1990
gdf1990.columns
gdf1990
fig, ax = plt.subplots(1,1)
gdf1990.plot(column="shdi",
           #scheme='BoxPlot' ('quantiles', 'FisherJenks')
           #cmap='OrRd', 
           legend=True,
           ax=ax)
plt.title('Distribución espacial del SHDI en 1990')
plt.tight_layout()
plt.show()
# Año 2021
gdf2021.columns
