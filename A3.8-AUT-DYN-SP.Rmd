---
title: 'TEMA 3: Dependencia espacial en los datos: econometría espacial'
author:
  name: Julián Ramajo, ramajo@unex.es
  affiliation: GRADO EN ESTADÍSTICA | ECONOMETRIA (502243)
subtitle: 'Aplicación 3.8: Precio de la vivienda en el condado de Lucas (Ohio, Estados Unidos)'
output:
  html_document:
    theme: journal
    highlight: haddock
...
#use_python("C:/Users/Usuario/AppData/Local/Programs/Python/Python310")
```

# Objetivo

En esta aplicación se introducen varios conceptos de estadística espacial, se realiza un análisis espacial de tipo exploratorio con la base de datos utilizada, y también se proponen modelos de regresión en los que las variables de dicho modelo y/o los errores del mismo presentan autocorrelación espacial.
...
library(spatialreg)
# Lectura de datos
house_sf <- geojson_sf("Ohio_USA.geojson")
class(house_sf)
str(house_sf)
summary(house_sf)
...
# Lectura de datos
house = gpd.read_file("Ohio_USA.geojson")
house.head(5)
