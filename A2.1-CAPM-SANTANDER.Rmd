---
title: 'Regresión lineal: Estimación MCO y contrastes de hipótesis'
author:
  name: Julián Ramajo, ramajo@unex.es
  affiliation: GRADO EN ESTADÍSTICA | ECONOMETRIA (502243)
subtitle: 'Aplicación 2.1: Modelo CAPM para las acciones del Banco Santander'
output:
  html_document:
    theme: journal
    highlight: haddock
...
donde $er\_SAN$ representa el *exceso de rendimiento* de las acciones del Banco Santander, definido como la diferencia entre la rentabilidad de las acciones del banco y la rentabilidad de un activo libre de riesgo, en nuestro caso las letras del tesoro, y $er\_IBEX$ es el exceso de rendimiento del mercado, representado por el índice IBEX35 de la bolsa española.

Como ejercicio, se pueden repetir todos los cálculos de la aplicación para las empresas Telefónica e Inditex.
...
library(tidyverse)
# Lectura de datos
CAPM_ESP <- read_delim("CAPM_ESP.csv", delim = ";")
head(CAPM_ESP)
tail(CAPM_ESP)
...
ebar <- mean(ehat)
sdehat <- sd(ehat)
hist(ehat, col="grey", breaks = 20, freq=FALSE, main="", ylab="density", xlab="ehat")
...
t_c
# Gráfico de la función de densidad de la distribución t de Student, del valor crítico 
...
# Gráfico de la función de densidad de la distribución F de Fisher-Snedecor, del valor crítico 
# y del estadístico F_0
...
import scipy as sp
# Lectura de datos
CAPM_ESP = pd.read_csv('CAPM_ESP.csv', delimiter=';', parse_dates=['date'], index_col='date')
CAPM_ESP.head()
CAPM_ESP.tail()
# Transformación de variables
def LogDiff(x):
...
plt.figure(4)
fig, ax = plt.subplots()
ax.hist(CAPM_SANTANDER.resid, bins=20, density=True, edgecolor='white', linewidth=1.2)
x = np.linspace(min(CAPM_SANTANDER.resid), max(CAPM_SANTANDER.resid), 100)
ax.plot(x, sp.stats.norm.pdf(x, ebar, sdehat), color='black', label = 'Distribución normal')
plt.title('Residuos del modelo CAPM para el Banco Santander')
plt.xlabel('Residuos')
plt.ylabel('Densidad')
plt.legend(loc='best')
...
t_0, 2*sp.stats.t.cdf(t_0, CAPM_SANTANDER.df_resid)
# Estadístico F y P-valor
H_0 = 'er_IBEX = 1'
F_0 = CAPM_SANTANDER.f_test(H_0)
print(F_0)
