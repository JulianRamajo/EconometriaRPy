...
$$log(pSAN_t) = \beta_1 + \beta_2 log(pIBEX_t)  + e_t$$

donde $pSAN$ es el precio de las acciones de Microsoft y $pIBEX$ es el índice bursátil IBEX 35.
...
reg <- lm(l_pSAN ~ l_pIBEX, data = data_daily)
summary(reg)
```
...
# Regresión lineal simple
reg = smf.ols('l_pSAN ~ l_pIBEX', data=data_daily).fit()
print(reg.summary())
```
